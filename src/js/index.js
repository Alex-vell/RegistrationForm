...
const onChangeValidation = (field, invalidBlock, value, type, condition = undefined) => {
  if (value.length === 0) {
    invalidBlock.innerText = 'Это обязательное поле';
    field.classList.add('error');
    return;
...
form.addEventListener('submit', async event => {
  ...
  Array.prototype.slice.call(form).forEach(field => {
    ...
    if (field.name === 'email') {
      onChangeValidation(email, invalidEmail, emailValue, 'email');
    }
...
email.addEventListener('input', e => {
  onChangeValidation(email, invalidEmail, e.currentTarget.value, 'email');
});
